...
set LOGFILE=C:\adwriter\verifier_logs\orchestrator_%STAMP%.log

rem UTF-8 so em dashes etc. never crash a redirected print; -u so the log fills as it runs
set PYTHONUTF8=1
echo [run_verifier] started %STAMP%  args: --all %*  > "%LOGFILE%"
C:\adwriter\adwriter-env\Scripts\python.exe -u C:\adwriter\verifier.py --all %* >> "%LOGFILE%" 2>&1
set RC=%ERRORLEVEL%
